...
class OpenMicroWave(BaseEnv):
    ...
    def __init__(self,cfg, sim_params, physics_engine, device_type, device_id, headless, log_dir=None):
        ...
        self.button_goalpos_offset_tensor = torch.zeros((self.asset_num, 3), device=self.device)
    ...
    def refresh_mechanism(self):
        update_index = torch.nonzero((self.open_door_stage == 1) & (self.mechanism_flag == 0) & (self.clock_wise == 1))
        # print(update_index)
        ...
        self.mechanism_flag = self.open_door_stage.clone()
    ...
    def _franka_init_pose(self):
        ...
        initial_franka_pose.r = gymapi.Quat(0,0,0.7071068, 0.7071068)

        initial_franka_pose.p = gymapi.Vec3(0.2, -0.4, 0.0)
        # initial_franka_pose.p = gymapi.Vec3(0.7, 0.0, 0.0)

        return initial_franka_pose

    def get_adjust_hand_pose(self):
        # button pos compute
        button_link_pos = self.part_rigid_body_tensor[:, :3].clone()
        button_link_rot = self.part_rigid_body_tensor[:, 3:7].clone()
        # handle pos compute
        handle_link_pos = self.rigid_body_tensor[:, :3].clone()
        handle_link_rot = self.rigid_body_tensor[:, 3:7].clone()

        down_q = torch.stack(self.num_envs * [torch.tensor([0.7071068, 0.7071068, 0, 0])]).to(self.device).view((self.num_envs, 4))
        button_q = torch.stack(self.num_envs * [torch.tensor([0, 1, 0, 0])]).to(self.device).view((self.num_envs, 4))

        # 0.7071068, 0, 0, 0.7071068 0, -0.7071068, 0.7071068, 0
        #button goalpos tensor
        button_goalpos_tensor = quat_apply(button_link_rot, self.button_goalpos_offset_tensor) + button_link_pos
        button_goalrot_tensor = quat_mul(button_link_rot, button_q)
        button_pose = torch.cat([button_goalpos_tensor, button_goalrot_tensor], dim=-1)
        ...
        return torch.cat([handle_pose, button_pose], dim=-1)
    ...
    def _load_obj(self, env_ptr, env_id):
        ...
        if self.obj_loaded == False :
            ...
            self.button_goalpos_offset_tensor = self.button_goalpos_offset_tensor.repeat_interleave(self.env_per_asset, dim=0)
    ...
    def _load_obj_asset(self):
        ...
        for id, (name, val) in asset_config_list:
            ...
            button_pos = torch.load(os.path.join(self.asset_root, self.dataset_path, str(val["name"]), "button_pos.npy"))
            handle_pos = torch.load(os.path.join(self.asset_root, self.dataset_path, str(val["name"]), "handle_pos.npy"))
            
            self.button_goalpos_offset_tensor[cur][0] = button_pos[0]
            self.button_goalpos_offset_tensor[cur][1] = button_pos[1]
            self.button_goalpos_offset_tensor[cur][2] = button_pos[2]
    ...
    def _obj_init_pose(self, min_dict, max_dict, name):
        ...
        cabinet_start_pose.p = gymapi.Vec3(0.0, 0.2, -min_dict[2]+0.3)
    ...
    def step(self, actions):
        ...
        #     self.cal_success()
        self.open_door_stage = ((torch.abs(self.two_dof_tensor[:, 0]) >= 0.7 * 
                                   (self.obj_actor_dof_upper_limits_tensor[:, 1] - self.obj_actor_dof_lower_limits_tensor[:, 1])))
        self.switch_flag = ((torch.abs(self.two_dof_tensor[:, 0]) >= 0.88 * 
                                   (self.obj_actor_dof_upper_limits_tensor[:, 1] - self.obj_actor_dof_lower_limits_tensor[:, 1])))
    ...
    def cal_success(self):
        self.open_handle_success = (torch.abs(self.two_dof_tensor[:, 0] - self.obj_lower_limits_tensor[:, 1]) > 0.1)
        self.open_door = (torch.abs(self.one_dof_tensor[:, 0]) > 0.025)
    ...
    def _perform_actions(self, actions):
        ...
        self.eff_act[:,-1] = -0.002
        self.pos_act[:,-4:-2] =  close * self.gripper + open * (1-self.gripper)
    ...
    def get_current_obs(self):
        ...
        hand_pc = self.compute_point_cloud_state(depth_bar = 2.5, type="hand")
        
        conn_pc = torch.cat([pc,hand_pc],dim=1)
        joints = self.franka_num_dofs - 2
        ...
        state = torch.cat([robotqpose, robotqvel], dim = -1)
        return conn_pc, state
