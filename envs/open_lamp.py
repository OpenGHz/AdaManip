...
class OpenLamp(BaseEnv):
    ...
    def _franka_init_pose(self):
        ...
        initial_franka_pose.p = gymapi.Vec3(0.7, 0.0, 0.1)
    ...
    def init_obj_dof_state(self, env_id):
        ...
        if self.task == "open_lamp":
            ...
            if target_cw == 1:
                ...
                random_upper = dof_props['upper'][1]
    ...
    def step(self, actions):
        ...
        self._refresh_observation()
        # self.cal_success()
        self.one_flag = 0.85 * (self.obj_actor_dof_upper_limits_tensor[:, 0] - self.obj_actor_dof_lower_limits_tensor[:, 0])
        self.two_flag = 0.85 * (self.obj_actor_dof_upper_limits_tensor[:, 1] - self.obj_actor_dof_lower_limits_tensor[:, 1])
