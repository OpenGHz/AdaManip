...
class OpenSafe(BaseEnv):
    ...
    def __init__(self,cfg, sim_params, physics_engine, device_type, device_id, headless, log_dir=None):
        ...
        self.gripper = torch.zeros((self.env_num,1), device=self.device) #False

        self.asset_root = cfg["env"]["asset"]["assetRoot"]
        self.asset_num = cfg["env"]["asset"]["AssetNum"]  # 1
        self.start_id = cfg["env"]["asset"]["StartID"]
        ...
        self.unlock = False
        self.seed = cfg["seed"]
        self.random_pose = cfg["env"]["randomPose"]
        self.pc_normalize_flag = cfg["env"]["pc_normalize"]
        ...
        self.dof_lower_limits_tensor = torch.zeros((self.asset_num, 3), device=self.device)
        ...
        self.handle_rigid_body_index = self.gym.find_actor_rigid_body_index(
            env_ptr,
            obj1_actor,
            self.handle_rigid_name,
            gymapi.DOMAIN_ENV
        )
        ...
        self.handle_rigid_body_tensor = self.rigid_body_tensor[:, self.handle_rigid_body_index, :]
        ...
        self.obj_actor_dof_max_torque_tensor = torch.zeros((self.num_envs, 3), device=self.device)
        self.obj_actor_dof_upper_limits_tensor = torch.zeros((self.num_envs, 3), device=self.device)
        self.obj_actor_dof_lower_limits_tensor = torch.zeros((self.num_envs, 3), device=self.device)
        self.get_obj_dof_property_tensor()

        self.dof_dim = self.franka_num_dofs + 3  # two dof mechanism
    ...
    def refresh_mechanism(self):
        update_open_index = torch.nonzero((self.open_door_stage == 1) & (self.mechanism_flag == 0) & (self.clock_wise > 0))
        update_close_index = torch.nonzero((self.open_door_stage == 0) & (self.mechanism_flag == 0) & (self.clock_wise == 0))
        for i in update_open_index:
            env = self.env_ptr_list[i]
            actor = self.actor_list[i]
            dof_props = self.gym.get_actor_dof_properties(env, actor)
            dof_props['upper'][0] = 1.578
            ...
            self.mechanism_flag[i] = 1
        for i in update_close_index:
            env = self.env_ptr_list[i]
            actor = self.actor_list[i]
            dof_props = self.gym.get_actor_dof_properties(env, actor)
    ...
    def _franka_init_pose(self):
        ...
        initial_franka_pose.p = gymapi.Vec3(0.7, -0.1, 0.0)

        return initial_franka_pose

    def get_adjust_hand_pose(self):
        # knob pos compute
        knob_link_pos = self.part_rigid_body_tensor[:, :3].clone()
        knob_link_rot = self.part_rigid_body_tensor[:, 3:7].clone()
        # handle pos compute
        handle_link_pos = self.handle_rigid_body_tensor[:, :3].clone()
        handle_link_rot = self.handle_rigid_body_tensor[:, 3:7].clone()
        ...
        #button goalpos tensor
        knob_goalpos_tensor = quat_apply(knob_link_rot, self.knob_goalpos_offset_tensor) + knob_link_pos
        knob_goalrot_tensor = quat_mul(knob_link_rot, down_q)
        knob_pose = torch.cat([knob_goalpos_tensor, knob_goalrot_tensor], dim=-1)

        handle_goalpos_tensor = quat_apply(handle_link_rot, self.handle_goalpos_offset_tensor) + handle_link_pos
        handle_goalrot_tensor = quat_mul(handle_link_rot, down_q)
        handle_pose = torch.cat([handle_goalpos_tensor, handle_goalrot_tensor], dim=-1)

        return torch.cat([handle_pose, knob_pose], dim=-1)
    ...
    def init_obj_dof_state(self, env_id):
        ...
        random_upper = (0.1*np.random.rand()+0.9)* dof_props['upper'][1]
        if self.task == "open_safe":
            ...
            if np.random.rand() > 0.5:
                ...
                dof_props["lower"][1] = -random_upper
            if self.clock_wise[env_id] > 0 : #np.random.rand() < self.cfg["env"]["clockwise"]:
                dof_props['upper'][0] = 0.0
                if dof_props["upper"][1] > 0:
            ...
            # dof_props['upper'][1] = random_upper
            dof_props["driveMode"] = (gymapi.DOF_MODE_EFFORT, gymapi.DOF_MODE_EFFORT, gymapi.DOF_MODE_EFFORT)
        else:
            print("Unrecognized task!\nTask should be one of: [xxxx]")
    ...
    def _load_obj(self, env_ptr, env_id):
        ...
        if self.obj_loaded == False :
            ...
            self.knob_goalpos_offset_tensor = self.knob_goalpos_offset_tensor.repeat_interleave(self.env_per_asset, dim=0)
            self.handle_goalpos_offset_tensor = self.handle_goalpos_offset_tensor.repeat_interleave(self.env_per_asset, dim=0)
    ...
    def _load_obj_asset(self):
        ...
        select_asset = [i+self.start_id for i in range(asset_len)] # start to load asset from the start id
        ...
        for id, (name, val) in asset_config_list:
            ...
            self.two_dof_name = list(dof_dict.keys())[1] #handle的joint_name
            # print("one_dof_name: ", self.one_dof_name, "two_dof_name: ", self.two_dof_name)

            rig_dict = self.gym.get_asset_rigid_body_dict(asset)
            assert (len(rig_dict) == 4) #body(base) board knob handle
            self.handle_rigid_name = list(rig_dict.keys())[3]
            ...
            assert (self.handle_rigid_name != "base")
            ...
            knob_pos = torch.load(os.path.join(self.asset_root, self.dataset_path, str(val["name"]), "knob_pos.npy"))
            handle_pos = torch.load(os.path.join(self.asset_root, self.dataset_path, str(val["name"]), "handle_pos.npy"))
            
            self.knob_goalpos_offset_tensor[cur][0] = knob_pos[0]
            self.knob_goalpos_offset_tensor[cur][1] = knob_pos[1]
            self.knob_goalpos_offset_tensor[cur][2] = knob_pos[2]

            self.handle_goalpos_offset_tensor[cur][0] = handle_pos[0]
            self.handle_goalpos_offset_tensor[cur][1] = handle_pos[1]
            self.handle_goalpos_offset_tensor[cur][2] = handle_pos[2]
            
            cur += 1

    def _obj_init_pose(self, min_dict, max_dict, name):
        # {"min": [-0.687565, -0.723071, -0.373959], "max": [0.698835, 0.605562, 0.410705]}
        # x = (np.random.rand()*2-1) * self.random_pose
        # y = (np.random.rand()*2-1) * self.random_pose
        front = 0.0
        height = 0.4
        if name == "sa3" or name=="sa5" or name == "sa16" or name == "sa6" or name == "sa8" or name == "sa17":
            height = 0.5
        elif name == "sa1" or name == "xx":
            height = 0.55
        elif name == "sa12" or name == "sa11" or name == 'sa20':
            height = 0.6
        elif name == "sa9" or name == "sa18":
            height = 0.3
        elif name == "sa10":
            height = 0.42
        elif name == "sa19" or name == "sa7":
            height = 0.2
        elif name == "sa4":
            height = 0.25
        # elif name == "sa15":
        ...
        cabinet_start_pose.p = gymapi.Vec3(front, 0.0, -min_dict[2]+height)
        # cabinet_start_pose.p = gymapi.Vec3(x, y, -min_dict[2]+height)
    ...
    def _partial_reset(self, to_reset="all", clock_same=True):
        ...
        for env_id, reset in enumerate(to_reset):
            ...
            if reset.item():
                ...
                rand_base_pos = (2 * torch.rand(2) - 1) * self.random_pose
                #torch.clamp(torch.normal(0,0.1,(2,)), -0.05, 0.05)
                reset_root_states[1][:2] = rand_base_pos               
        ...
        self.gripper = torch.zeros((self.num_envs,1), device=self.device) #False
        self.actions = torch.zeros((self.num_envs, self.num_actions), device=self.device)
        if reseted:
    ...
    def step(self, actions):
        ...
        self.open_door_stage = (self.clock_wise>0)*((torch.abs(self.two_dof_tensor[:, 0]) >= 0.85 * 
                                   (self.obj_actor_dof_upper_limits_tensor[:, 1] - self.obj_actor_dof_lower_limits_tensor[:, 1])))\
                                + (self.clock_wise==0)*((torch.abs(self.two_dof_tensor[:, 0]) <= 0.15 * 
                                   (self.obj_actor_dof_upper_limits_tensor[:, 1] - self.obj_actor_dof_lower_limits_tensor[:, 1])))
    ...
    def _perform_actions(self, actions):
        # set action in the manipulation class
        ...
        self.eff_act[:,-3] = -2
        self.pos_act[:,-5:-3] =  close * self.gripper + open * (1-self.gripper)
    ...
    def pc_normalize(self, pc):
        ...
        pc = pc - center
        # print(torch.max(torch.sqrt(torch.sum(pc**2, axis=2)), axis=1, keepdim=True)[0])
