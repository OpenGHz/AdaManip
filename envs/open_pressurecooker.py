...
class OpenPressureCooker(BaseEnv):
    ...
    def __init__(self,cfg, sim_params, physics_engine, device_type, device_id, headless, log_dir=None):
        ...
        # min random_range * range_scale * open_stage_scale --> 0.824 * 0.5 * scale
        self.try_range = 0.824 * 0.5 * self.open_stage_scale
    ...
    def _franka_init_pose(self):
        ...
        initial_franka_pose.p = gymapi.Vec3(0.6, 0.0, 0.0)
    ...
    def get_adjust_hand_pose(self):
        ...
        down_q = torch.stack(self.num_envs * [torch.tensor([0.5, 0.5, -0.5, 0.5])]).to(self.device).view((self.num_envs, 4))
    ...
    def init_obj_dof_state(self, env_id):
        ...
        if self.task in ("open_pressurecooker", "open_pressurecooker_one_go"):
            ...
            if use_clockwise:
                # clock wise — handle locked: rotate is required before lift.
                ...
                # counter clock wise — handle already openable, lift
    ...
    def _obj_init_pose(self, min_dict, max_dict, name):
        ...
        cabinet_start_pose.p = gymapi.Vec3(0.0, 0.0, -min_dict[2]+0.3)
