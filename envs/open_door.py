...
class OpenDoor(BaseEnv):
    ...
    def __init__(self,cfg, sim_params, physics_engine, device_type, device_id, headless, log_dir=None):
        ...
        # min_random * range_scale * open_stage_scale --> 0.525 * scale
        # (preserves the historical 0.44625 at scale=0.85).
        self.try_range = 0.525 * self.open_stage_scale
    ...
    def _partial_reset(self, to_reset="all", clock_wise_override=None):
        ...
        self.gripper = False
        self.actions = torch.zeros((self.num_envs, self.num_actions), device=self.device)
        # Clear override after partial reset so subsequent calls without an
        # override fall back to the random sampling path.
