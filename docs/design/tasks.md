...
- **无冗余步骤链**：
  - N×顺时针旋转把手 -> 拉开窗户
  - N×逆时针旋转把手 -> 拉开窗户
*(注：与 Safe 一步到位的旋钮不同，Window 的把手 dof 需要旋转到 ≥0.85 × 范围才会触发 `open_bottle_stage`，所以一次成功开启可能对应不同次数的旋转；N 由 demo 实际累积的正确方向旋转次数决定。)*
...
- **无冗余步骤链**：
  - N×顺时针旋转把手 -> 拉开门
  - N×逆时针旋转把手 -> 拉开门
*(注：与 Safe 一步到位的旋钮不同，Door 的把手 dof 需要旋转到 ≥0.85 × 范围才会触发 `open_bottle_stage`，所以一次成功开启可能对应不同次数的旋转；N 由 demo 实际累积的正确方向旋转次数决定。)*
