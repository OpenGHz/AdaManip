...
def add_noise(p, q, p_noise_level=0.01, q_noise_level=0.01):
    # 对三维向量添加噪声
    # 使用torch
    noise = torch.normal(0, p_noise_level, p.shape).cuda()
    p_noisy = p + noise

    # 对四元数添加噪声
    noise = torch.normal(0, q_noise_level, q.shape).cuda()
    q_noisy = quat_unit(q + noise)

    return p_noisy, q_noisy