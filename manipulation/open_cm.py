...
class OpenCoffeeMachineManipulation(BaseManipulation) :
    ...
    def ground_truth_chain_for_collect(
        ...
        return self.ground_truth_chain_from_intrinsic_n(
            env_id=env_id,
            state=state,
            n_min_attr="_cm_intrinsic_n",
            rotate_op="旋转手柄",
            lift_op="拉动手柄",
            success_hint="pulled the coffee-machine handle",
        )

    def concrete_attempt_chain_for_collect(self, env_id: int, state: Dict[str, Any]):
        chains = getattr(self, "_cm_attempt_chains", None)
        statuses = getattr(self, "_cm_stage_statuses", None)
        if (
            chains is not None
    ...
    def collect_manip_data(self):
        ...
        for eps in range(eps_num):
            ...
            self.env.reset()
            self.collect_episode_start(ctx, eps)
            self._cm_attempt_chains = [[] for _ in range(self.env.num_envs)]
            self._cm_stage_statuses = [[] for _ in range(self.env.num_envs)]
            self._cm_intrinsic_n = [None] * self.env.num_envs
            self._cm_cum_rot = [0] * self.env.num_envs
            ...
            def _flush(env_id, success):
                ...
                if op == "旋转手柄":
                    ...
                    self._cm_stage_statuses[env_id].append(bool(success))
            ...
            for t in range(max_step):
                ...
                for env_id in range(self.env.num_envs):
                    if not done_flag[env_id]:
                        ...
                        op = "拉动手柄" if res_per_env[env_id] == "z" else "旋转手柄"
                        if current_op[env_id] is None:
                            ...
                            current_op[env_id] = op
                            current_count[env_id] = 1
                        step_idx_for_env[env_id] = len(self._cm_attempt_chains[env_id])
